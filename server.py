...
import re
import requests # Keep for Session object
...
# FIX: Create a single session object to persist cookies and headers
...
def fetch_video_details(url):
    try:
        ...
        # It's good practice to use the final URL after redirects
        ...
        # Return the final URL along with other details
...
def get_or_generate_thumbnail(link):
    with cache_lock:
        ...
        try:
            # Use the same session object for this request
...
# Other functions (load_cache, save_cache, parse_videos) remain the same as the previous corrected version...
def load_cache():
...
# --- FLASK ROUTES (with minor adjustments) ---
    ...
    with open(VIDEO_FILE, "a", encoding="utf-8") as f:
        for url in urls:
            ...
            try:
                ...
                # Store the final, stable URL
                entry = f"{title} | {','.join(final_tags)} | {final_url}\n"
                f.write(entry)
                
                # Use the final URL for thumbnailing too
...
# remove_video and the main execution block remain the same...
